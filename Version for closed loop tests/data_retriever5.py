...
def custom_timestamp_to_Unix(custom_timestamp_int):
    # Define the origin of the custom timestamp (midnight UTC, 1 January 1 AD) in Unix timestamp format.
    custom_timestamp_origin_seconds = -62135596800  # Seconds since Unix epoch to 1 January 1 AD

    # Convert the custom timestamp from milliseconds to seconds.
    custom_timestamp_seconds = custom_timestamp_int / 1000

    # Calculate the Unix timestamp in seconds.
    unix_timestamp_seconds = custom_timestamp_seconds + custom_timestamp_origin_seconds
        ...
        for file in files:
            if file.endswith(".zip"):
                ...
                try:
                    ...
                    print(f"Warning: {file} is not a valid zip file and will be skipped.")
...
def get_columns_list_h5(columns_list, files_list, time_precision=2):
    ...
    # Initialize a set to accumulate missing columns
    missing_columns = set()
    ...
    for file_info in tqdm(files_list, desc='Processing files'):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Initialize valid columns with columns_list if not None
            valid_columns = columns_list if columns_list is not None else []
            ...
            for chunk in hdf_iterator:
                if columns_list is None:
                    ...
                    # Filter the columns to include only those that exist in the chunk
                    existing_columns = [col for col in columns_list if col in chunk.columns]
                    missing_columns.update(set(valid_columns) - set(existing_columns))

                    if existing_columns:
                        chunk_df = chunk[existing_columns]
    ...
    # Print missing columns in one line
    if missing_columns:
...
def average_data_by_interval(data_df, sync_interval=None):
    if sync_interval is not None:
        ...
        # Print some sample rows to check the binning
        #print("Sample Rows with 'bin' Column:")
        #print(data_df.head())
        ...
        if 'time' in data_df.columns:
            # Use the custom_timestamp_to_Unix function to convert 'bin' values to Unix timestamps
            data_df['time'] = data_df['bin'].apply(custom_timestamp_to_Unix)
...
#print(datetime.utcfromtimestamp(max_time))
#print(datetime.utcfromtimestamp(custom_timestamp_to_Unix(results_df['timestamp'].max())))
